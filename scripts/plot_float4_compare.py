"""
plot_float4_compare.py
----------------------
Generates performance comparison plots for CPU and GPU kernel benchmarks (global, shared, float4).

- Loads benchmark results from CSV
- Plots: execution time, GPU kernel comparison, and speedup vs CPU
- Saves all figures to the 'benchmarks/' directory
...
def main():
    """
    Loads results, computes speedups, and generates/saves performance plots.
    """
    # Load CSV results — column names match benchmark_utils.cpp output
    ...
    df_melted = pd.melt(df, id_vars=["N"], value_vars=[
        "cpu_ms", "gpu_global_ms", "gpu_shared_ms", "gpu_float4_ms"
    ])
    sns.boxplot(data=df_melted, x="N", y="value", hue="variable", showfliers=False)
    ...
    plt.figure(figsize=(12, 6))
    df_gpu = pd.melt(df, id_vars=["N"], value_vars=[
        "gpu_global_ms", "gpu_shared_ms", "gpu_float4_ms"
    ])
    sns.boxplot(data=df_gpu, x="N", y="value", hue="variable", showfliers=False)
    ...
    plt.figure(figsize=(12, 6))
    df_speedup = pd.melt(df, id_vars=["N"], value_vars=[
        "speedup_global", "speedup_shared", "speedup_float4"
    ])
    sns.boxplot(data=df_speedup, x="N", y="value", hue="variable", showfliers=False)
