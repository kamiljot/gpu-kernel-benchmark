...
int main(int argc, char* argv[])
{
    ...
    if (!backend)
    {
        ...
        std::cerr << "Available: ";
        for (const auto& name : BackendRegistry::instance().available_backends()) std::cerr << name << " ";
        std::cerr << std::endl;
        return 1;
    }
    ...
    if (!scenario)
    {
        ...
        std::cerr << "Available: ";
        for (const auto& name : ScenarioRegistry::instance().available_scenarios()) std::cerr << name << " ";
        std::cerr << std::endl;
        return 1;
    }
